...
tab1_layout = [
    [
        sg.Column([
            ...
            [sg.Checkbox("Hide data inside Media", key='media_checkbox', default=True, enable_events=True)],
            ...
            [sg.Text(text="media file", key='media_file_title')],
            [sg.Column([[sg.Input(key='media_file', disabled_readonly_background_color='black'), sg.FileBrowse(key='media_file_browse', file_types=(('All Files', '*.*'), ))]], key='media_file_row')],
            [sg.Text(text='', key='media_file_message', size=(40, 2))]
...
window = sg.Window("Hack Inversion", layout)
while True:
    window.refresh()
    ...
    if event == sg.WIN_CLOSED:
        ...
        if values['media_checkbox']:
            if not os.path.exists(values['media_file']):
                window['media_file_message'].update("Invalid PNG file", text_color='red')
        try:
            if values['rsa_radio']:
                if not values['media_checkbox']:
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']):
                        ...
                        if values['text_radio']:
                            output_path = f"{values['output_dir']}/{stego.randonnumber}"
                            ...
                            output_path = f"{values['output_dir']}/{os.path.basename(values['file'])}.{stego.randonnumber}"
                            encrypted = crypto.encrypt_rsa(file_data, values['public_key'], f"rsa_file:{os.path.basename(values['file'])}")
                            save_file(output_path, encrypted)
                            window['encrypt_message_text'].update("Successfully encrypted", text_color='green')

                elif values['media_checkbox']:
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']) and os.path.exists(values['media_file']):
                        window['output_dir_message'].update('')
                        window['public_key_message'].update('')
                        window['media_file_message'].update('')

                        if values['text_radio']:
                            encrypted = crypto.encrypt_rsa(values['message'], values['public_key'], 'rsa_text')
                            stego.encode_data(encrypted, values['media_file'], f"{values['output_dir']}/temp/{stego.randonnumber}.png")
                            window['encrypt_message_text'].update("Successfully encrypted", text_color='green')
                ...
                if not values['media_checkbox']:
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']):
                        ...
                        if values['text_radio']:
                            output_path = f"{values['output_dir']}/temp/{stego.randonnumber}"
            ...
            elif values['no_encryption_radio'] and values['media_checkbox']:
                if os.path.exists(values['output_dir']) and os.path.exists(values['media_file']):
                    window['output_dir_message'].update('')
                    window['media_file_message'].update('')

                    if values['text_radio']:
                        stego.encode('plain_text\u2028' + values['message'], values['media_file'], f"{values['output_dir']}/temp/{stego.randonnumber}.png")
                        window['encrypt_message_text'].update("Successfully encrypted", text_color='green')

                    elif values['file_radio']:
                        with open(values['file'], 'rb') as input_file:
                            file_data = input_file.read()
                        stego.encode(f"plain_file:{os.path.basename(values['file'])}\u2028".encode('utf-8') + file_data, values['media_file'], f"{values['output_dir']}/temp/{stego.randonnumber}.png")
                        window['encrypt_message_text'].update("Successfully encrypted", text_color='green')
        ...
        try:
            ...
            if values['encrypted_file'].split('.')[-1].lower() == 'avi':
    ...
    if values['no_encryption_radio']:
        window['media_checkbox'].update(True, disabled=True)
        window['media_file'].update(disabled=False)
        window['media_file_browse'].update(disabled=False)
        window['public_key'].update(disabled=True)
        window['public_key_browse'].update(disabled=True)

    elif not values['no_encryption_radio']:
        window['media_checkbox'].update(disabled=False)
        window['media_file'].update(disabled=False)
        window['media_file_browse'].update(disabled=False)
    ...
    elif not values['media_checkbox'] and not values['no_encryption_radio']:
        window['media_file'].update(disabled=True)
        window['media_file_browse'].update(disabled=True)
